		...
		<body>
				...
				<section id="banner" class="major">
					<div class="inner">
						<header class="major">
							...
							<!-- ex) smhrd님 환영합니다.  -->
							<!-- 로그인을 수행 전/후 에 따라 h1태그 출력을 생각해보기 -->
							
							<!-- 로그인 전 -> 즉, session 영역에 member가 없다면~ -->
							<!-- empty = null 과 같은 의미 -->
							<c:if test="${ empty member }">
								<h1>로그인 한 세션아이디를 출력해주세요</h1>
							</c:if>
							<!-- 로그인 후 -> 즉, session 영역에 member가 있다면~ -->
							<c:if test="${ not empty member }">
			...
			<script type="text/javascript">
				...
				function checkEmail() {
				    ...
				    // 2. ajax 통신 시작
				    $.ajax({
				    	url : 'checkEmail',  		 // 1) 요청 url
                    	type : 'get',       		 // 2) 요청 방식
                    	data : {inputEmail : email}, // 3) 서버로 전달할 데이터
                    	
                    	// 4) 요청 성공 시 실행할 함수
                    	success : function(res) {  
                    		// result 변수에는 서버에서 응답받은 데이터가 담겨서 옴
                    		if(res == 'true') {
                    			$("#resultEmail").text("사용 가능한 이메일입니다.");
                    		} else {
                    			$("#resultEmail").text("이미 사용 중인 이메일입니다.");
                    		}
                    	},
                    	// 5) 요청 실패 시 실행할 함수
                    	error : function() {  
