						...
						<table>
							...
							<c:forEach var="m" items="${list}">
                                <tr>
                                    ...
                                    <td>${m.tel}</td>
                                    <td>${m.address}</td>
                                    <td><a href="delete?email=${m.email}" onclick="return confirm('정말 삭제하시겠습니까?');">x</a></td>
                                    <!-- 쿼리스트링 : 사용자가 입력 데이터를 url 주소에 포함시켜 전송하는 방식 -->
                                    <!-- 쿼리스트링의 시작은 주소 이후에 ? 뒤 부터이다. -->			
                                    <!-- 삭제 버튼 클릭 시, 해당 데이터의 사용자 email을 요청값으로 같이 전달 -->
                                </tr>
                            </c:forEach>
						</table>
					</nav>		
					<a href="/web/" class="button next scrolly">되돌아가기</a>	
			</div>
		<!-- Scripts -->
